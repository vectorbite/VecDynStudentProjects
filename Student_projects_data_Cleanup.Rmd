...
Download the entire Vectorbase PopBio database here, this database contains lots of mosquito data-sets from around the world. There are two files in this folder, one which contains the main data and another publication information and metadata. You can connect them together using the shared ID codes.

    - https://imperiallondon-my.sharepoint.com/:f:/g/personal/mwatts_ic_ac_uk/ElBCZN-hJtlKoM-N5R7SDnUBe9vLkrhhXtkJ-IEjjLmPMA?e=lR86y4
...
### Data-sets that do not provide GPS trap coordinates. 

You may find that some data-sets do not provide GPS coordinates for trap station sites  which presents us with a problem. This is the case for the KBS049:Aphid Collection Tower KBS data. At the time of writing, I'm still waiting for the lead contact to reply to me regarding where we can source spatial data to compliment this data-set.  

However, we can still use the date-set by linking the station location sites with the Census.gov US Cartographic Boundary Shape files. You can download this data-set here.  https://www.census.gov/geo/maps-data/data/cbf/cbf_ua.html.  

Download the 2010 Census zip file, unzip and place the unzipped folder into VecDynStudentProjects/files/  

You will need to install the following packages for the next steps. 
...
If you are using Ubuntu you may need to install the following packages before you can use the spatial packages:   
...
Load up the us_census data-set which contains shape files that match geo data in the KBS data-set

```{r}
us_census_sites <- st_read("./files/cb_2012_us_uac10_500k/cb_2012_us_uac10_500k.shp")
```

We don't need all the information in the US census data-set so lets only keep the necessary columns

```{r}
us_census_sites <- dplyr::select(us_census_sites, NAME10, geometry)

```

Let's take a look at the us_census data. Each grey shape represents a small urban area, we'll use these shape files to capture climate data. If you look in the data frame you'll see that each place name corresponds to spatial information captured in the geometry column

```{r}
tm_shape(us_census_sites, ylim = c(20,50), xlim = c(-130,-65)) +
    tm_polygons()

```

Download the KBS aphid data-set here

https://lter.kbs.msu.edu/datatables/122.csv

You'll first need to edit it appropriately in excel or equivalent to get it into the right format. 

```{r}

KBSaphid <- read_csv("./files/122-aphid+colonization+via+suction+traps.csv")
```

You'll next need to join the two data-sets together using the place names, the naming format differs in each data-set, so we'll need to do some formatting first. 

```{r}

KBSaphid <- 
  KBSaphid  %>%
  tidyr::unite("sample_location", c("station","state"), sep = ", ", remove = FALSE)
  
```

We will also create a data frame of distinct locations.

```{r}

KBSaphid_collection_sites <- 
  KBSaphid  %>%
  dplyr::select(sample_location) %>%
  dplyr::distinct()
  
```

Next lets create a query so we can join the KBSaphid collection stations data and us_census spatial information 

```{r}

KBSaphid_collection_sites_sp <- dplyr::inner_join(us_census_sites, KBSaphid_collection_sites, by = c("NAME10" = "sample_location"))

```

The query only captured spatial information for 14 stations out of a possible 41, however this is enough to work with for now. 

We can take a look to see what we captured from the us_census data-set. It is a bit difficult to see anything if we try to look at them all together since the geographical areas are so small. But these spatial entities are now connected to the KBSaphid sample stations. 

```{r}
tm_shape(KBSaphid_collection_sites_sp, ylim = c(20,50), xlim = c(-130,-65)) +
    tm_polygons()
```

OK, next we need to create spatial points which will capture climate information within each of the 14 stations.

We can generate 50 points, these are randomly distributed within each station shape, however the provision of points is dependent on the the station shape size. Larger areas get more points. Note that we should use as many sample points as possible to represent the station,  but we'll be restricted by memory and processing time so we'll keep this tutorial simple and only use a few points.

```{r}

sample_points <- st_sample(KBSaphid_collection_sites_sp, 50) %>% # random points, as a list ...
  st_sf()  # ... to data frame ...

```

OK, so lets have a look what we have done. 
If we filter KBSaphid_collection_sites_sp and choose one sampling station we can see that we have a shape file representing the area of the station where aphids were sampled. We randomly assigned points to the area which we will use to extract climate information. 

```{r}
t <- dplyr::filter(KBSaphid_collection_sites_sp, NAME10 == "Manhattan, KS")
...
```
Now lets re-join our random sample points back to our station names. 

```{r}
climate_sampling_points <- st_join(sample_points, KBSaphid_collection_sites_sp, join = st_intersects)
```

The next thing we want to do is join the 'sample points + station names' back to the dates in the main data-set.

First we need to create a data-set of unique sample sites and dates

```{r}

KBSaphid_collection_sites_dates <- 
  KBSaphid  %>%
  dplyr::select(sample_date, sample_location) %>%
  dplyr::distinct()
  
```

Now lets query this with the climate sample points we made for each sample_location. 

```{r}
KBSaphid_collection_sites_dates_with_random_points <- dplyr::inner_join(climate_sampling_points, KBSaphid_collection_sites_dates, by = c("NAME10" = "sample_location"))

```

Now if you look at the KBSaphid_collection_sites_dates_with_random_points, you will see that we have all of the dates in the original study along with random points we created from the station shapes.  

Next we want to spread the sampling dates out throughout the day. So we can try to get an accurate reading of the weather for that day.

Create a time variable, starting at 00.01, then every 6 hours. 

```{r}
KBSaphid_collection_sites_dates_with_random_points_00  <- 
  KBSaphid_collection_sites_dates_with_random_points %>%
  dplyr::select(sample_date, NAME10, geometry) %>%
  dplyr::mutate(sample_time = "00:00:01") %>%
  tidyr::unite("sample_datetime", c("sample_date","sample_time"), sep = " ", remove = FALSE) %>%
  dplyr::mutate(sample_datetime = as.POSIXct(sample_datetime, format = "%Y-%m-%d%H:%M:%S"))

KBSaphid_collection_sites_dates_with_random_points_06  <- 
  KBSaphid_collection_sites_dates_with_random_points %>%
  dplyr::select(sample_date, NAME10, geometry) %>%
  dplyr::mutate(sample_time = "06:00:01") %>%
  tidyr::unite("sample_datetime", c("sample_date","sample_time"), sep = " ", remove = FALSE) %>%
  dplyr::mutate(sample_datetime = as.POSIXct(sample_datetime, format = "%Y-%m-%d%H:%M:%S"))

KBSaphid_collection_sites_dates_with_random_points_12  <- 
  KBSaphid_collection_sites_dates_with_random_points %>%
  dplyr::select(sample_date, NAME10, geometry) %>%
  dplyr::mutate(sample_time = "12:00:01") %>%
  tidyr::unite("sample_datetime", c("sample_date","sample_time"), sep = " ", remove = FALSE) %>%
  dplyr::mutate(sample_datetime = as.POSIXct(sample_datetime, format = "%Y-%m-%d%H:%M:%S"))

KBSaphid_collection_sites_dates_with_random_points_18  <- 
  KBSaphid_collection_sites_dates_with_random_points %>%
  dplyr::select(sample_date, NAME10, geometry) %>%
  dplyr::mutate(sample_time = "18:00:01") %>%
  tidyr::unite("sample_datetime", c("sample_date","sample_time"), sep = " ", remove = FALSE) %>%
  dplyr::mutate(sample_datetime = as.POSIXct(sample_datetime, format = "%Y-%m-%d%H:%M:%S"))

KBSaphid_all <- rbind(KBSaphid_collection_sites_dates_with_random_points_00,KBSaphid_collection_sites_dates_with_random_points_06,KBSaphid_collection_sites_dates_with_random_points_12, KBSaphid_collection_sites_dates_with_random_points_18)

```

Now we are ready to retrieve information to our KBSaphid_all points and time data frame.  However, we need to reformat the spatial information for the RNCEP package. by turning the geometry column into individual points. 


```{r}

##create a function that allows us to convert spatial point data to numerical point locations
...
KBSaphid_all <- sfc_as_cols(KBSaphid_all)


```

we need to drop the spatial data for the next step, however keep the lat (y) long (x) data, as this is just numerical data.
lets take a subset of the data for the sake of saving time

```{r}
KBSaphid_all_no_geom <- 
  KBSaphid_all %>%
  st_set_geometry(NULL) %>%
  distinct()

KBSaphid_Monroe <- dplyr::filter(KBSaphid_all_no_geom, NAME10 == "Monroe, MI")
 
```

lets gather the temperature data from the NCEP

```{r}

KBSaphid_Monroe$temp <- NCEP.interp(variable='air.sig995', level='surface', lat=KBSaphid_Monroe$y, lon=KBSaphid_Monroe$x, dt=KBSaphid_Monroe$sample_datetime, reanalysis2=FALSE, keep.unpacking.info=TRUE)

```

At this point you could keep repeating the above step, extracting other climate variables for the NCEP database e.g. relative humidity (‘rhum.sig995’ Relative Humidity), which can be used as a proxy for rainfall. 

We now have a number of temperature data points for each station at a particular date-time. Note that we filtered station == "Monroe, MI" so we only extracted values for that locations.

Next lets take an average of the 6 hourly daily climate + the point locations in each station, creating a daily average temperature for each station. Note you could also add other climate variables to the summarize function.  

```{r}
KBSaphid_Monroe_av_daily_station_temp <-
KBSaphid_Monroe %>% 
  dplyr::group_by(NAME10, sample_date) %>%
  dplyr::summarise(avg_daily_station_temp = mean(temp))
```

And now lets create a query that merges the temp data we just gathered back into the main data set. Again note that we have filter for the station 'Monroe' so it you won't retrieve all the values in the original data set, only those that match with this station.  

```{r}
KBSaphid_with_temps <- 
  dplyr::inner_join(KBSaphid, KBSaphid_Monroe_av_daily_station_temp, by = c("sample_location" = "NAME10", "sample_date"="sample_date"))
```

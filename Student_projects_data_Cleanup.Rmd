...
For your projects you will be dealing with abundance data, which can be visualized as simple time-series plot. The Y access represents the abundance of something at a particular point in time, in our case it will be a species of insect, and the X access represents when sample was measured. 

```{r time-series-exmple, echo=FALSE, fig.align = "center", out.width = "100%", out.extra='', fig.ext='png'}
knitr::include_graphics('./files/time-series-plot.png')

```

## Overview of data collection

Along with the time series data, we need minimal amounts of information,  that can help us make informed decisions about the quality of the data e.g. was it measured accurately using the best equipment and do the methods used to select the sample represent the population in question.
...
You can find the GDPP user guide in files/GPDD_User_Guide.pdf. 

The GDPP data will pose a slight challenge when converting them to the VecDyn format as the data format as the GPDD does not store exact
sampling dates,  instead observations are temporally coded. you'll need to code this back into time stamped dates. more info below. 
...
Your challenge will be to clean-up and process data-sets, mapping them to the vecdyn template. You can find the R code below which will help you build a vecdyn template. 
...
Note that all data frame fields are created in character format,  you'll need to convert them to the correct formats at a later stage. 

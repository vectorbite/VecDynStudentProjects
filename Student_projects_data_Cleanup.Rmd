...
Download the entire vectorbase popbio database here
...
This package contains functions that retrieve, organize, and visualize weather data from the NCEP/NCAR Reanalysis (http://www.esrl.noaa.gov/psd/data/gridded/data.ncep.reanalysis.html) and NCEP/DOE Reanalysis II (http://www.esrl.noaa.gov/psd/data/gridded/data.ncep.reanalysis2.html) datasets. Data are queried via the Internet and may be obtained for a specified spatial and temporal extent or interpolated to a point in space and time. 

For this package, we need to convert the relevent data to numerical format. 
...
Unfortunately the RNCEP doesnt allow us (as far as i know) to extract daily averages based on dates alone, we can only extract values at particular date times to try to give us a representive indication of the temperature / rainfall that day. We need to create sampling points spread throughout the day to get daily averages from the NCEP/NCAR Reanalysis dataset. 

With this dataset, a sample start and end date is provided. lets extarct values beginning from the start data and ending at the end date over a 24 hour period. 
...
```
We now have temperature data for each trap site at a particualr datetime. Note that we filtered sample_location == "Mullins" so we don't have all the trap locations in this subset of data. But if you didn't filter that location you'd end up with all trap locations and dates within the data-set. 
...
You may find that some data-sets do not provide GPS trap coordinates,  which presents us with a problem. This is the case for the KBS049:Aphid Collection Tower KBS data. At the time of writing, i'm still waiting for the lead contact to reply to me regarding where we can source spatial data to compliment this data-set.  

However, we can still use the date-set by linking the sample location sites with the Census.gov US Cartographic Boundary Shapefiles. You can download this data-set here.  https://www.census.gov/geo/maps-data/data/cbf/cbf_ua.html.  
...
Load up the us_census dataset which contains shape files that match geo data in the KBS data-set

```{r}
us_census_sites <- st_read("./files/cb_2012_us_uac10_500k/cb_2012_us_uac10_500k.shp")
```
we don't need all the information in the US cencus data-set so lets only keep the necesary columns
...
Let's take a look at the us_census data. Each grey shape represents a small urban area, we'll use these shapefiles to capture climate data. If you look in the data frame you'll see that each place name correspeonds to spatial infomration captured in the geometry column
...
You'll first need to edit it approriately in excel or equivlent to get it into the right format. 
...
You'll next need to join the two data-sets together using the place names, the naming format differs in each data-set, so we'll need to do some formatting first. We will also create a data frame of distinct locations.
...
Next lets create a query so we can give the KBSaphid collection stations spatial information 
...
We can take a look to see what we captured from the us_census dataset. It is a bit difficult to see anything if we try to look at them all together since the geographical areas are so small. But these spatial entities are now connected to the KBSaphid sample stations. 
```{r}
tm_shape(KBSaphid_collection_sites_sp, ylim = c(20,50), xlim = c(-130,-65)) +
    tm_polygons()
```

Ok, next we need to create spatial points which will capture climate information within each of the 14 station.

We generate 50 points, these are randomly distributed within each station shape, however the provision of points is dependent on the the station shape size. Larger areas get more points. Note that we should use as many sample points as possible to represent the station,  but we'll be restricted by memory and processing time so we'll keep this tutorial simple.
...
If we filter KBSaphid_collection_sites_sp and choose one sampling station we can see that we have a shape file representing the area of the station where the aphids were sampled. We randomly asigned points to the area which we will use to extract climate information. 
...
The next thing we want to do is join the 'sample points' back to the main data-set.

We first need to create a  dataset of unique sample sites and dates
...
```
Now if you look at the KBSaphid_collection_sites_dates_with_random_points, you will see that we have all of the dates in the orginal study along with random points we created from the station shapes.  

Next we want to spread the sampling dates out through the day. So we can repeat something similar to what we did with the mosquito data. 
...
Now we are ready to retrieve information to our KBSaphid_all points and time data frame.  However, we need to reformat the spatial information for the RNCEP package. by turning the geomerty column into individual points. 


```{r}

##create a function that allows us to converts spatial point data to points
...
```
lets take a subset of the data merely for processing purposes, otherwise it will take all day
```{r}
KBSaphid_all_no_geom <- 
  KBSaphid_all %>%
  dplyr::mutate(sample_date = as.Date(sample_date, format = "%Y-%m-%d")) %>%
...
Next you want to take an average of all the tempertaure samples, 
https://cran.r-project.org/web/packages/RNCEP/RNCEP.pdf
You can also repeat the steps for the relative humidity (‘rhum.sig995’ Relative Humidity), which can be used as a proxy for rainfall. 

Once you have download all the required data per sample location and time, you can average them by place name and date to get all the desired climate

You can then requery this with the original data set to get all the climate variables. 